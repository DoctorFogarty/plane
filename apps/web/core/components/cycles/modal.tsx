...
export function CycleCreateUpdateModal(props: CycleModalProps) {
  ...
  const dateChecker = async (selectedProjectId: string, payload: CycleDateCheckData) => {
    const res = await cycleService.cycleDateCheck(workspaceSlug, selectedProjectId, payload);
    return res.status;
  };
