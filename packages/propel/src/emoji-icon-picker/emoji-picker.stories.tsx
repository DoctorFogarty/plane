...
export const CloseOnSelectDisabled: Story = {
  ...
  render() {
    const [isOpen, setIsOpen] = useState(false);
    const [selectedValues, setSelectedValues] = useState<Array<TChangeHandlerProps & { id: string }>>([]);

    const handleChange = (value: TChangeHandlerProps) => {
      setSelectedValues((prev) => [...prev, { ...value, id: crypto.randomUUID() }]);
    };
          ...
          <div className="rounded-sm border border-subtle bg-layer-1 p-4 text-13">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedValues.map((val) => (
                <span key={val.id} className="text-16">
    ...
    return (
      <div className="max-w-md p-4">
        <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-subtle p-6">
          <div>
            <label htmlFor="emoji-picker-project-title" className="mb-2 block text-13 font-medium">
              Project Title
            </label>
            <input
              id="emoji-picker-project-title"
              type="text"
          ...
          <div>
            <div className="mb-2 text-13 font-medium">Project Icon</div>
            <EmojiPicker
              isOpen={isOpen}
              handleToggle={setIsOpen}
